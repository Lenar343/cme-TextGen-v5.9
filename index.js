...
async function getTitles() {
  ...
  try {
    const response = await fetch('https://lenmaybe345.app.n8n.cloud/webhook/get-titles', {
      method: 'POST',
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('dropdowntitles').addEventListener('change', async function () {
    ...
    try {
      const response = await fetch('https://lenmaybe345.app.n8n.cloud/webhook/get-latest-prompt', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function postText() {
  ...
  try {
    const response = await fetch('https://lenmaybe345.app.n8n.cloud/webhook/generate-from-text', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function continueAI(inputName) {
  ...
  try {
    const response = await fetch('https://lenmaybe345.app.n8n.cloud/webhook/continue-text', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formData)
    });
...
async function generateAI(inputName) {
  ...
  try {
    const response = await fetch('https://lenmaybe345.app.n8n.cloud/webhook/generate-new-text', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formData)
    });
...
async function deleteRecord() {
  ...
  try {
    const response = await fetch('https://lenmaybe345.app.n8n.cloud/webhook/delete-record', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ formData })
    });
